...
export default function Dashboard() {
    ...
    // ✅ Added useRef to prevent duplicate API calls
    ...
    const fetchDashboardData = async () => {
        ...
        try {
            ...
            // ✅ Only show toast if there's an actual error
            if (error?.response?.data?.error || error?.message) {
                toast.error(error?.response?.data?.error || error.message)
            }
        } finally {
            // ✅ Fixed typo and ensures loading stops
            setLoading(false)
        }
    }
    ...
    return (
        <div className=" text-slate-500 mb-28">
            ...
            <h2 className="text-xl font-medium text-slate-700 mt-8">Total Reviews</h2>
